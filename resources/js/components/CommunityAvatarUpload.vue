...
const avatarUrl = computed(() => {
    ...
    return props.community.avatar ? `/storage/${props.community.avatar}` : null;
});

const fallbackInitial = computed(() => {
    return props.community.name.charAt(0).toUpperCase();
});
...
const uploadFile = (file: File) => {
    ...
    router.post(`/communities/${props.community.id}/avatar`, formData, {
        forceFormData: true,
        preserveScroll: true,
        onFinish: () => {
...
const deleteAvatar = () => {
    router.delete(`/communities/${props.community.id}/avatar`, {
        preserveScroll: true,
    });
};
</script>
    ...
    <div class="flex flex-col items-center space-y-4">
        <div class="relative">
            <Avatar class="h-24 w-24">
                <AvatarImage :src="avatarUrl" :alt="`Avatar de ${community.title}`" />
                <AvatarFallback class="text-lg bg-blue-500 text-white">
                    {{ fallbackInitial }}
                </AvatarFallback>
            </Avatar>
